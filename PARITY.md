...
| 8 | Session-26 visual fixes: Overview BOQ filters all-zero placeholder codes (44→25 rows); Cash Flow "$0 Committed" legend hidden when empty; Project Health "committed vs budget" descriptor | session 26 | Descriptor ✅ (2026-07-08); Overview BOQ zero-code filter ✅ shipped with the R1 rebuild (e2e asserts code 003 hidden on Overview). Remaining: Cash Flow legend — re-check when gap 14 closes in R3 | Medium | ➖ |
