...
| 9 | DUPLICATE_PROJECT — reducer action exists, no UI button | session 29 inventory | Action present, no button on ProjectsList. Note 2026-07-08: legacy shows no visible Duplicate control either (the string only appears in its inline script) — closing this is additive UI, not strictly parity | Low | ⬜ |
...
| 13 | **Project Overview tab structure** — legacy leads with 5 stat blocks (CONTRACT VALUE w/ target margin · ORIGINAL BUDGET w/ % spent · APPROVED VARIATIONS · TRUE OVERRUN · OUTSTANDING INVOICES) + the full analytic BOQ table (CODE→STATUS, zero-placeholder codes filtered 44→25, ON BUDGET/OVER statuses). Port shows a different KPI set (COST TO DATE / CURRENT MARGIN / MARGIN EROSION) + Contract-vs-Cost & Project-info panels, no BOQ table (its analytic table lives on the BOQ tab instead) | 0-A → session 26 (`D1`) | Different page entirely | High | ⬜ |
...
| 17 | **Contract-value semantics** — legacy contract value = budget + margin (dashboard "$0.7M contract value"; Overview $273,566 vs budget $232,531; Open Book lists "Margin target 15%" + "Original contract value" as separate lines, adjusted $352,279). Port treats raw budget as contract ($0.5M; $311,244 = budget + variations). Estimating shows the same disease: legacy lists sell totals ($172,500 / $634,146.34), port lists raw cost ($138,000 / $520,000). Open Book cost-to-date also differs ($268,357 "invoices + POs" vs $320,918) | sessions 0-A → 13 | Financial semantics diverge on every money surface | **Critical** | ⬜ |
| 18 | **Claims retention units bug** — seed stores `retention.rate: 5` (percent, `sitework/src/state/seed.ts:2751`) but `ClaimsTab.tsx` computes `amount × rate` expecting a fraction (its own doc says 0.05) → −$225,000 retention on a $45,000 claim, negative Net Certified (legacy: −$2,250 / $47,025). Wrong output visible on every Claims row | n/a (port bug found in 2026-07-08 audit) | Broken | **Critical** | ⬜ |
...
| PC & PS | "PC & PS" page title + margin-on-excess explainer, "+ Add PC/PS Item" buttons (gap 5), inline totals footer line, grey RECONCILED, Net-to-Claim = excess-based (port shows actual+margin: $11,800 total vs legacy $5,200 — semantics, verify with gap 17) | TOTALS table row style |
...
| R0 | **Financial semantics core** — legacy money maths transliterated into `computeFinancials.ts`; retention units bug; unit tests pin legacy's exact on-screen numbers | 17, 18 | ⬜ |
