...
- ✅ **R0 Financial semantics core** (Critical) — legacy money maths into `computeFinancials.ts` (contract value = budget ÷ (1−margin), true overrun, cost-to-date, PC/PS net-to-claim, dashboard portfolio KPIs) + retention percent-unit fix; 157 unit / 34 e2e green; Dashboard, Overview, PC&PS, Claims, Open Book verified rendering :8766's exact numbers — gaps 17, 18. Session 49
- ✅ **R1 Project Overview + BOQ edit tab** (High) — Overview rebuilt as `D1`+`D1v2` verbatim (5 stat blocks, analytic BOQ table w/ zero-code filter, Contract-vs-Cost KPI panel + Duplicate Project); BOQ tab rebuilt as the `w1` edit surface (expandable line-items, `Gc` live committed, amber VO sub-tables, reorder/edit/delete, `g1` LineItemForm); duplicate reducer arm transliterated to `Z1` (planning status, money history cleared). 158 unit / 34 e2e green; side-by-side pairs verified — gaps 13, 16, 8a, 9. Session 50
- ✅ **R2 Money tables** (High) — Invoices/POs/Claims rebuilt from `O1v2`/`M1v2`/`Cl1`: full column sets (Doc Ref, cost-code descriptions, GST, Total inc GST), filter chips, zebra rows, totals footers, inline RECEIVE, Claims stat blocks; `POFormV2` + `InvFormV2` fields transliterated (subcontractor selects, docRef, GST preview, poNum auto-number, POs start `pending`). 158 unit / 34 e2e green; three pairs verified vs :8766 — gap 12 (3 rows). Session 51
- ✅ **R3 Cash Flow** (High) — full `j1`+`j1v2` transliteration: stat blocks ($173,530/$52,988/$120,542), stacked Monthly Cost Outflows chart + legend, month table w/ paid-only cumulative, Forward Cash Flow Forecast ($220,578 spread over 12 months, conditional Committed legend = gap 8b). Pair verified vs :8766; 158 unit / 34 e2e green — gaps 14, 8b. Session 52
- ✅ **R4 Settings** (Critical) — `St1` transliterated in full: business identity, default contract type/margin/GST, home state, 8 state licences, 9 insurance registrations, Save Settings + Saved! flash, Reset to Demo Data w/ verbatim confirm; project-form defaults wired from Settings (e2e-pinned). 158 unit / 35 e2e green; pair + text dump verified vs :8766 — gaps 2, 3. Session 53
- ✅ **R5 Leads kanban + Estimating + Clients/Subs tables** (High) — `G1` kanban + lead detail + Convert-to-Project restored (`Cv1`, `Z1` convert semantics, `quoted` stage back); `H1` list + drill-in + `h1`/`b1` forms + `Z1` promote semantics; `L1` expandable table; `V1` compliance table (pill drift fixed, 60-day window). 158 unit / 35 e2e green; four pairs verified vs :8766 — gaps 15 + 12 (3 rows). Session 54
- ✅ **R6 Variations requestedBy + PC/PS forms** (High) — `v1` form (Requested By, Reason Other, both conditional comments) + `B1` table (Requested By column, count sub-line); `pcf`/`psf` as shared `PcPsForm` (validation, kind statuses, Delete) + full `Pcps` anatomy (title/explainer, Add buttons, inline totals, grey RECONCILED). 158 unit / 35 e2e green; pairs verified vs :8766 — gaps 4, 5 + gap-12 (2 rows). Session 55
- ✅ **R7 Remaining tab anatomy** (Medium) — eight surfaces transliterated: Timesheets (`U1`/`C1`), Defects (`k1v2` **incl. the `k1` Retention & FFC module the port never had**), Schedule (`_1`/`S1`), RFIs (`RF1`/`RFIForm1`), Selections (`N1`/`T1` + approve dialog), Diary (`R1`/`A1`), Calendar (`Calx`), Open Book (`Obx` anatomy) + client modal (`m1` labels). 158 unit / 35 e2e green; pairs verified vs :8766 — gap 12 fully closed. Session 56
- ✅ **R8 Help & Education + final sweep** (Medium) — `X1` transliterated (4 tabs, 3 guides w/ numbered steps, 6 module docs w/ Pro Tips, 14-term glossary, Rawlinson Rates w/ location-factor bars; `wi` content verbatim). 158 unit / 36 e2e green; full 26-pair sweep regenerated. **All 18 PARITY gaps closed** — awaiting Jake's walkthrough sign-off → root URL swap + branch-stack merge. Session 57
- ✅ **Phase 4.6 closeout** — Jake's side-by-side walkthrough (:5173 vs :8766) signed off; R0–R8 stack merged to main with a merge commit (`9d37b0b`, per-session commits preserved for bisect/revert); full gate suite re-run at the tip pre-merge (format/lint/typecheck/158 unit/build/36 e2e, all green); Vercel deploy verified — root URL now serves the rebuilt Vite app, baseline remains at `/legacy`. **Phase 4.6 complete.** Next: resume Phase 4.5 B–F, then the design phase. Session 58
...
- ✅ **4.5-B Money correctness** — central `src/lib/money.ts` (`GST_RATE`, `roundCents` w/ sub-cent epsilon bias, `gstOf`/`incGst`, `parseAmount`); every inlined GST computation (`×0.1`/`×1.1`) across Invoices/POs/Claims tables + forms + Estimating routed through the helpers; `computeFinancials` claim helpers delegate to them; the `Number(x)||0`/`parseFloat(x)||0` idiom replaced by `parseAmount` across ~25 form fields (currency-tolerant, explicit fallbacks). **Bug fixed:** `TaxInvoicePrint` treated `invoice.amount` as inc-GST while every table treats it as ex-GST — the printed tax invoice disagreed with the on-screen total; now consistent (amount = ex-GST). 12 new money unit tests (170 total); typecheck/lint/build/36 e2e green. NOT a cents migration (Phase 5 does that in Postgres). Session 59
- ✅ **4.5-C Linking layer** — `EntityLink` + `FilterBanner` primitives; `?cc=`/`?sup=` drill filters on Invoices & POs (BOQ committed figure → invoices filtered by code; supplier cell self-filters; cost-code cell → BOQ), `?client=` on the Projects list (Clients row project-count → client-filtered projects); Dashboard Active Projects → /projects and Compliance Alerts → /subcontractors tiles clickable (StatBlock gained an optional `to`). Filtered footers now sum the shown rows. All render-time lookups off the URL — nothing persisted. 3 new e2e (39 total); typecheck/lint/build/170 unit green. Session 60
...
| 46 | 2026-07-05 | 4.6 | **P1 — project form + statutory validation shipped (PARITY gap 1, Critical).** Legacy `I0` extracted verbatim from the baseline by string-search and ported: `ProjectForm.tsx` (all 10 fields incl. cost-plus-only Estimated Value, exact select options), "+ New Project" on Projects list + ghost "Edit Project" beside the contract-type badge on Overview (both trigger points match legacy). Statutory rules live in pure `src/lib/statutory.ts` (mirrors substantiation.ts): `vicS13Blocked`, `qldHwsAckRequired`, `depositCapText` with all 8 states' copy pinned verbatim by 21 unit tests. Fidelity note: legacy Save is NOT greyed when blocked — it stays clickable and a refused save flips `attempted` (red-lines) — ported exactly, correcting the ROADMAP's own "greyed Save" prose. 7 new e2e specs: create happy path, empty-name red-line, VIC block + renovation exemption lift + $1m boundary lift, fixed-price never blocked, QLD ack gate, edit-from-overview. 144 unit / 32 e2e green; all three statutory states verified in browser (screenshots). Legacy `sw_ct`/`sw_state` Settings defaults noted for P2. |
...
| 49 | 2026-07-08 | 4.6 | **R0 — financial semantics core shipped (gaps 17 + 18, both Critical).** Legacy money maths extracted from the baseline source (`D1`/`D1v2`/`Y1`/`Cl1`/`Pcps`/`Obx`) and transliterated into `computeFinancials.ts`: contract value = budget ÷ (1 − margin/100) (markup on sell — resolves the $0.5M-vs-$0.7M dashboard delta), adjusted CV = contract + approved vars ($352,279), true overrun = Σ code committed − adjusted budget ($33,119), committed/cost-to-date = invoices≠Rejected + POs≠cancelled with cost = max(committed, paid) ($268,357), PC/PS net-to-claim = variance + margin-on-excess (+$1,800 / $5,200 / $5,000), dashboard portfolio KPIs (Y1: live-projects scope, Approved-only invoices, 17% portfolio margin). Retention bug root-caused as a UNITS mismatch: legacy stores percent (5) and divides by 100 at use — the port read a fraction. Fix keeps percent in the data (sw_state_v1-compatible), adds `claimRetention`/`claimNetCertified` (× 1.1 GST on retained net → −$2,250 / $47,025), rewires ClaimsTab + both print routes. Discovery: legacy `D1v2` has a Duplicate Project button on Overview (gap 9 corrected). Exact :8766-rendered numbers pinned by unit tests (text-dumped from the live baseline, not screenshots); 157 unit / 34 e2e green; all five money surfaces verified in-browser against :8766. |

| 50 | 2026-07-08 | 4.6 | **R1 — Project Overview + BOQ edit tab rebuilt from legacy source (gaps 13, 16, 8a, 9).** Overview thrown away and rebuilt as `D1`+`D1v2` verbatim: contract-type chip under the title, five content-width stat blocks over 1px ink rules (Contract Value w/ target margin · Original Budget w/ 138% spent · Approved Variations violet · True Overrun pink "Above adj. budget" · Outstanding Invoices w/ $53K paid), the analytic BOQ table with the session-26 zero-placeholder filter (44→25 rows, e2e-pinned), per-code approved-variation overlays, abs-value overrun display matching legacy `k()`, `Ma` health badges, then the Contract-vs-Cost 4-KPI panel ($352,279 / $268,357 / 23.8% / +8.8% with accent rules) and the **Duplicate Project** button legacy has there — reducer arm transliterated to `Z1` semantics (copy → `planning`, invoices+variations cleared, "(Copy)" suffix; `ProjectStatus` union gains `planning`). BOQ tab thrown away and rebuilt as the `w1` edit surface: expandable code rows with `Gc` live committed (invoices≠Rejected + POs≠cancelled per code, `codeDocTotals`), "/ budget" pair coloured by health, ↑↓✎× actions with legacy confirm copy, line-items tables (`formatCurrencyExact` = legacy `K0`), `g1` ported as `LineItemForm`, amber Approved Variations sub-tables with totals, legacy dialog titles (Add/Edit Cost Code, Import BOQ Template, Add Line Item). Port inventions removed (Overview Project-info panel, read-only BOQ table w/ invented totals row). 158 unit / 34 e2e green; parity pairs regenerated and verified. |

| 51 | 2026-07-09 | 4.6 | **R2 — money tables rebuilt from legacy source (gap 12: Invoices, POs, Claims).** Invoices = `O1v2` verbatim: "$52,988 paid · $120,542 approved · $173,530 total" stat line, All/Pending/Approved/Paid filter chips (ink-filled active), Supplier/Doc Ref/full cost-code/Date/Due/Amount ex GST/GST/Total inc GST columns, zebra rows, "7 invoices · Total inc GST: $190,883" footer — including legacy's Comments-header quirk preserved bug-for-bug (status renders under the Comments column exactly as :8766 does). POs = `M1v2` verbatim: PO # (poNum), Doc Ref, description, full cost-code text, GST columns, "4 POs · $104,309" footer, inline RECEIVE on any non-received/cancelled PO. Claims = `Cl1` verbatim: "claimed of contract" sub-line, TOTAL CLAIMED/PAID TO DATE/OUTSTANDING stat blocks, Claim/Due Date columns, GST + Total inc GST, "$289,022" footer matching :8766's rounding. Forms transliterated: `POFormV2` (subcontractor select auto-fills supplier, Document Reference, GST preview strip, poNum "PO-00N" auto-number, NO status field — new POs start `pending` per legacy) and `InvFormV2` deltas (Subcontractor select, Document Reference, amount label corrected to ex-GST). Types gain legacy fields (poNum/docRef/subId); PurchaseStatus transliterated ('pending' default; draft/sent deprecated). Port's invoice ID column dropped for column parity; DOCS flags, print icons, Date column, Retention cert kept per gap-12. 158 unit / 34 e2e green; three side-by-side pairs verified. |

| 52 | 2026-07-09 | 4.6 | **R3 — Cash Flow rebuilt from legacy source (gaps 14 + 8b).** The port's single-month-bar invention thrown away; the page is now `j1`+`j1v2` verbatim. `j1`: "Cost outflows by month" header, three stat blocks (Total Committed = paid + approved invoices $173,530 · Paid to Date $52,988 green · Outstanding $120,542 violet), the Monthly Cost Outflows stacked bar chart (paid green under approved lilac w/ #C7D2FE border, 28px bars, Paid + Approved/Outstanding legend) over the fixed seed-era Sep-25→Jun-26 window (legacy hardcode, kept bug-for-bug), and the month table (rows only where total > 0; **Cumulative is a paid-only running sum** — $5,023 → $38,815 → $52,988 exactly as :8766). `j1v2`: Forward Cash Flow Forecast — $220,578 remaining · 12-month outlook, committed per month = approved invoices due + un-received POs dated (#2563EB) with remaining contract spread across empty months (#BFDBFE), value labels ("18k"), month baseline rules, and the three mini stats (Total Committed 12mo / Forecast Remaining / Total Projected Outflow). Gap 8b verified live: with zero committed in the current window the "Committed" legend entry is hidden, matching legacy's conditional. 158 unit / 34 e2e green; pair verified vs :8766. |

| 53 | 2026-07-10 | 4.6 | **R4 — Settings rebuilt from legacy source (gaps 2 + 3).** `St1` transliterated in full: ruled `stSec` sections with boxed inputs (legacy Settings uses bordered fields, not the underline primitive), Business Name / Your Name / Default Contract Type (Cost Plus·Fixed Price) / Default Margin (%) / GST "Registered for GST" checkbox / Home State (8 states) / mono ABN, Builder Licences by State (8 fields) and Insurance Registration Numbers (9 fields: HBCF NSW, DBI VIC, VBA VIC, QBCC QLD, HII WA, BII SA, ACT Fidelity, TAS + NT no-scheme) with verbatim explainer copy. Save Settings + "Saved!" flash (no reload — the port's reducer state is reactive, unlike legacy's per-key localStorage). **Reset to Demo Data** (gap 3): red-outline button, legacy's verbatim confirm, clears `sw_state_v2` + `sw_state_v1`, reloads to seed. Settings type formalised (defaultContractType / defaultMarginPct / gstRegistered / homeState / licences / insurance). **Project-form wiring closed**: legacy `I0` seeds contractType/state from `sw_ct`/`sw_state` — `blank(settings)` now reads defaultContractType/homeState (margin stays hardcoded 15, exactly as legacy I0 does), pinned by a new e2e (set Fixed Price + VIC in Settings → + New Project opens pre-filled). This also clears gap 1's deferred note. 158 unit / 35 e2e green; screenshot pair + below-fold text dump match :8766 field-for-field. |

| 54 | 2026-07-10 | 4.6 | **R5 — Leads kanban, Estimating, Clients + Subs rebuilt from legacy source (gaps 15 + three gap-12 rows).** Leads = `G1` verbatim: "Lead Pipeline · 4 active · $2.1M", five colour-coded kanban columns (Prospect/Tendering/**Quoted**/Won/Lost — `LeadStage` regains 'quoted'), per-column $K totals, hover cards w/ source chips, lead detail drill-in (stage select, Estimated Value/Source/Follow-up stats, notes) and the **restored Convert-to-Project flow** — `Cv1` client-picker dialog + `CONVERT_LEAD_TO_PROJECT` transliterated to `Z1` (project built in the reducer: live, margin 20, settings-seeded contract type/state, lead value as estimatedValue; lead stamped `convertedToProjectId`) — the port had the action but NO dispatching UI. Estimating = `H1` verbatim: "2 estimates · 0 won" sub-line, "+ New Estimate" (`h1` form: name/client/address/target margin), underline tabs w/o counts, sell totals in legacy AUD mixed precision ($172,500 / $634,146.34), estimate drill-in (Cost Budget / Contract (margin) / GST stats, codes table, `b1` + Add Code, Edit, Promote); `PROMOTE_ESTIMATE` transliterated (named after estimate, Σ-budget estimatedValue, flips estimate to won, no name prompt). Clients = `L1`: CLIENT/CONTACT/mono ABN/PROJECTS ruled table with expand-in-place detail + Edit Client. Subs = `V1`: "6 registered · 5 compliance issues", uppercase trade chips, six-column table (mono licence, bare-text PL/WC statuses + dates, SWMS), lilac non-compliant tint, 60-day expiry window — kills the last PV pill drift. 158 unit / 35 e2e green (specs reworked for kanban/expand flows); four pairs verified vs :8766. |

| 55 | 2026-07-10 | 4.6 | **R6 — Variation requestedBy + PC/PS forms rebuilt from legacy source (gaps 4 + 5).** Variations: `v1` form transliterated in full — editable mono Variation ID (auto "VO-00N"), **Requested By** select (Owner/Builder/Architect/Other, default Owner — `VariationRequestedBy` type added), Cost Code "code — desc", Status (Pending/Approved/Rejected, legacy has no Disputed), **Reason Category regains 'Other'** with spaced labels, Time Impact (days), and both conditional comment fields with verbatim placeholders ("Describe the reason for this variation" / "Specify who requested this"); `B1` table rebuilt — 26px header, "7 variations · $78,712 approved · $1,200 pending" sub-line (violet pending), ID / Description / Requested By / full Cost Code / Amount / Status / Date, port's raw-enum Reason column removed. PC & PS: `pcf`/`psf` transliterated as a shared `PcPsForm` (Description required, Allowance > 0, Margin Rate 0.2-default, Actual Cost, kind-specific status sets incl. Selected/Procured and InProgress/Complete, red-outline Delete with verbatim confirm), row-click edit, and the page anatomy completed — "PC & PS" title + margin-on-excess explainer, 18px section heads with "+ Add PC/PS Item", inline right-aligned totals footers (Allowance/Actual/Net-to-Claim, green net), grey RECONCILED (legacy `He` default — StatusBadge remapped). New e2e coverage: conditional comment fields + pcf validation/happy-path. 158 unit / 35 e2e green; both pairs verified vs :8766. |

| 56 | 2026-07-11 | 4.6 | **R7 — remaining tab anatomy rebuilt from legacy source (gap 12 closed).** Eight surfaces transliterated: **Timesheets** `U1`+`C1` (cost-code column, "23h logged · $2,005 labour cost", 8h/$95/hr units, Log Time form); **Defects** `k1v2` — including the **`k1` Retention & FFC module the port never had**: Retention Summary kv rows ($315,000 / 5% / $15,750 held violet) with the purple held-vs-maximum progress bar, Completion Timeline (Practical Completion → FFC → End of DLP with release amounts) and the green "Issue FFC Certificate" action (UPDATE_RETENTION), then the Defects & PC Checklist (pills, green rectified dates, Edit links; DefectForm gains In Progress status + legacy labels/titles); **Schedule** `_1`+`S1` (2px green-gradient progress bar, SVG status dots full/dashed/hollow, per-node Edit, Save Milestone form); **RFI Register** `RF1`+`RFIForm1` (overdue pink sub-line + row tint + pink Required-By, Responded date column, Edit links, full form); **Client Selections** `N1`+`T1` (awaiting-decision sub-line, note lines, Approve on open AND pending, add + approve-option dialogs, `SelectionStatus` regains 'open'); **Site Diary** `R1`+`A1` (weekday/day/month date-block cards, incident blue borders + violet tags, green sub chips, New Diary Entry form with sub toggle chips + incident checkbox); **Calendar** `Calx` (events counter, mono month headers, date-first rows, EXPIRED/≤30d pills, certificate expiries added); **Open Book** `Obx` anatomy (OPEN-BOOK REPORT eyebrow, generated line, boxed cards, six legacy section titles, claims Total approved/paid, Retention rate/held kvs, footer note); **client modal** `m1` (Company / Client Name, stacked full-width fields, Save Client). Retention type gains contractValue/pcDate/ffcDate/dlpMonths. 158 unit / 35 e2e green; pairs verified vs :8766. |

| 57 | 2026-07-11 | 4.6 | **R8 — Help & Education rebuilt + final sweep (gap 6 closed — ALL 18 GAPS CLOSED).** `/education` placeholder replaced with the full `X1` transliteration: four tab chips (Getting Started / Module Guides / Glossary / Rawlinson Rates), 280px list + detail layouts, three getting-started guides with numbered ink-circle steps, six module docs with Pro Tip callouts (3px ink left rule), the 14-term glossary grid, and the Rawlinson Rates tab (handbook history, Coverage trade list, Regional Location Factors with green/violet gradient bars and the NT +25% remote premium). Every word of the legacy `wi` content blob carried verbatim into `educationContent.ts`. New e2e walks all four tabs (guide switching, Pro Tip, 14 terms, location factors). 158 unit / 36 e2e green; help pair pixel-identical; full 26-pair sweep regenerated in `sitework/parity-shots/`. **The rebuild program (R0–R8) is complete: every PARITY.md gap row is ✅.** Remaining ceremony: Jake's side-by-side walkthrough sign-off, then swap the root URL back to the Vite app and merge the R-branch stack to main. |

*Last updated: 2026-07-11 (session 57 — R8 done, ALL GAPS CLOSED; next: Jake's sign-off walkthrough → root swap + merge)*
