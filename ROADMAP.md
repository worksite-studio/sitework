...
| 28 | 2026-05-24 | 1.5 | Phase 1.5 item 2: cost-plus invoice + progress claim substantiation. File picker on InvFormV2 / POFormV2 / ClaimForm1; save blocked on cost-plus without a doc. Trust-fragile session — 3 rounds of fixes consolidated into one audit-driven batch commit (CC dropdown empty-state, claimNo state-init, Cl1 edit-modal + list-cell positional fallback). Verified in browser preview before merge (CC dropdown opens with 44 options, both blocks fire, existing claims show #1–#4, no console errors). Phase 1.5 closeout now in progress for items 3–5 |

*Last updated: 2026-05-24 (session 28 — substantiation shipped; closing out Phase 1.5 with BOQ CC fix → BOQ template import → insurance cert upload → warranty calendar → owner-facing open-book report; then migration handoff docs)*
